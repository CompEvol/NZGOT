...
//    };
    ...
    private static final String chainLength = "400000000";
    private static final String logEvery = "400000";
    private static final String fileName = "113g46t.log";
    private final TREE_PRIOR treePrior;
    private double popSize = 300000;
    private double clockRate = 1.0E-8;
    ...
    public static void main(String[] args) throws IOException {
        Path workPath = Paths.get(System.getProperty("user.home"),
                "Projects/BEAST2/Adelie/", NameUtil.getNameNoExtension(fileName));
        if (args.length == 1) {
            workPath = Paths.get(args[0]);
        }
        System.out.println("workPath = " + workPath);

        Path inPath = Paths.get(workPath.toString(), NameUtil.getNameNoExtension(fileName) + ".raw.xml");
        ...
        Path outPath = Paths.get(workPath.toString(), NameUtil.getNameNoExtension(fileName) + ".xml");
        ...
        XMLGenerator xmlGenerator = new XMLGenerator(TREE_PRIOR.ConstantPopulation);
        out.println(xmlGenerator.getBeastHead());

        String[] alignmentAndTrait = xmlGenerator.getAlignmentAndTraitFrom(reader);
        String alignments = alignmentAndTrait[0];
        String traits = alignmentAndTrait[1];

        System.out.println("traits = " + traits);

        String[] taxa = xmlGenerator.getTaxa(alignments);
        String[] ids = xmlGenerator.getGeneIds(alignments);

        out.println(xmlGenerator.getTaxonSet(taxa));
        out.println(alignments);

        xmlGenerator.createXML(out, ids, traits, traitName);
    ...
    public void createXML(PrintStream out, String[] ids, String beastHead, String alignments,
    ...
    public String[] getGeneIds(String alignments){
        // need tidy up terrible BEUti xml to make this pattern works
        String patternString = "<data id=\"(.+?)\" name=\"alignment\"";

        List<String> geneList = StringUtil.matchedSubstring(alignments, patternString);

        System.out.println("Find gene ids = " + geneList);

        return geneList.toArray(new String[geneList.size()]);
    }

    public String[] getTaxa(String alignments){
        String oneAlignment = alignments.substring(alignments.indexOf("<data"), alignments.indexOf("</data>"));
        String patternString = "taxon=\"(.+?)\" totalcount=";

        List<String> taxaList = StringUtil.matchedSubstring(oneAlignment, patternString);

        System.out.println("Find taxa = " + taxaList);

        return taxaList.toArray(new String[taxaList.size()]);
    }

    public String getBeastHead() {
    ...
    // NOT include Beast Head
    public String[] getAlignmentAndTraitFrom(BufferedReader reader){
        ...
        try {
            ...
            while (line != null) {
                ...
                if (line.contains("<trait id=")) {
                    isTrait = true;
                    line = reader.readLine(); // start from next line
                }
                if (line.contains("<taxa id="))
                    isTrait = false;

                if (isTrait)
                    trait+=line.replaceAll("\t", "").trim() + "\n";
        ...
        return new String[]{stringBuilder.toString(), trait};
    }
    ...
    private String getMCMC(){
        return "\t<run chainLength=\"" + chainLength + "\" id=\"mcmc\" spec=\"MCMC\">\n" +
                "\t\t<state id=\"state\" storeEvery=\"50000\">\n";
    }

    private String getTrees(String id, String idref, String traits, String traitName) {
    ...
    private String getOtherParameters(String firstId){
        return "\t\t\t<parameter id=\"clockRate.c:" + firstId + "\" name=\"stateNode\">" + clockRate + "</parameter>\n" +
    ...
    private String getRandomTree(String id, String idref) {
        if (id.contentEquals(idref)) {
            return "\t\t<init estimate=\"false\" id=\"RandomTree.t:" + id + "\" initial=\"@Tree.t:" + id + "\"\n" +
                    ...
                    "\t\t\t\t<parameter id=\"randomPopSize.t:" + id + "\" name=\"popSize\">" + popSize + "</parameter>\n" +
                    "\t\t\t</populationModel>\n" +
                    "\t\t</init>\n";
        } else {
            return "\t\t<init estimate=\"false\" id=\"RandomTree.t:" + id + "\" initial=\"@Tree.t:" + id + "\"\n" +
    ...
    private String getDistributionHead(){
        return "\t\t<distribution id=\"posterior\" spec=\"util.CompoundDistribution\">\n" +
                "\t\t\t<distribution id=\"prior\" spec=\"util.CompoundDistribution\">\n";
    }

    private String getTreeDistribution(String id, String idref) {
    ...
    private String getLikelihoods(String id, String idref){
        if (id.contentEquals(idref)) {
    ...
    private String getDistributionEnd(){
        return "\t\t\t</distribution>\n" +
                "\t\t</distribution>\n";
    }

    private String getTreeOperators(String id) {
        return "\t\t<operator id=\"treeScaler.t:" + id + "\" scaleFactor=\"0.8\" spec=\"ScaleOperator\"\n" +
                "\t\t\ttree=\"@Tree.t:" + id + "\" weight=\"3.0\"/>\n\n" +
                "\t\t<operator id=\"treeRootScaler.t:" + id + "\" rootOnly=\"true\" scaleFactor=\"0.7\" spec=\"ScaleOperator\"\n" +
                "\t\t\ttree=\"@Tree.t:" + id + "\" weight=\"3.0\"/>\n\n" +
    ...
    private String getOtherOperators(String firstId){
    ...
    private String getLogs(String firstId){
    ...
    private String getTreeLogs(String id) {
