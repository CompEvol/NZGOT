...
//        intervals.initByName("tree", tree);
    ...
    // hard code
    public SiteModel getSiteModel(Alignment dummyAlg, double mutationRate, double kappa, double gammaShape) throws Exception {
        ...
        siteModel.initByName("mutationRate", Double.toString(mutationRate), "gammaCategoryCount", 4,
...
//        };
        String[] genes = new String[40];
        for (int i=0; i<genes.length; i++) {
            genes[i] = "gene" + Integer.toString(i+1);
        }
        ...
        createXML(workPath, "40g10k6t0sim.xml", genes, sequenceLength, popSize, taxa, dates, clockRate, mutationRate, kappa, gammaShape);
...
//        mutationRate = 1.0;
        ...
        createXML(workPath, "40g10k6t716sim.xml", genes, sequenceLength, popSize, taxa, dates, clockRate, mutationRate, kappa, gammaShape);
    } // main

    // create xml
    private static void createXML(String workPath, String outFileName, String[] genes, int sequenceLength, double popSize,
                                  String[] taxa, int[] dates, double clockRate, double mutationRate, double kappa, double gammaShape) {

        // tree file
        String treeFileName = NameUtil.getNameNoExtension(outFileName) + ".trees.txt";
        Path outTreePath = Paths.get(workPath, treeFileName);
        PrintStream outTreeFile = null;
        try {
            outTreeFile = new PrintStream(outTreePath.toFile());
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
        assert outTreeFile != null;

        out.println("Total " + genes.length + " genes");
...
//                out.println("tree " + id + " = " + tree.toString());
                ...
                Alignment alignment = treeSeqSimulator.getSimulatedAlignment(dummyAlg, tree, siteModel, branchRateModel, sequenceLength);
                alignment.setID(id);
